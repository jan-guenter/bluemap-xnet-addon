...
"""Generate the bounded deterministic XNet visual gallery."""
...
def verify_function() -> bytes:
    ...
    lines.append(
        "tellraw @a {\"text\":\"XNet visual gallery checked\","
        "\"color\":\"green\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    return {
        ...
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 XNet visual gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"XNet visual gallery loaded; "
